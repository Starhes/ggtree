...
const Experience: React.FC = () => {
  const { isMobile } = useContext(TreeContext) as TreeContextType;
  const countScale = isMobile ? 0.2 : 1;
  ...
  return (
    ...
    >
      ...
      {/* 慢速闪烁的金色星星 */}
      <Sparkles count={Math.floor(300 * countScale)} scale={25} size={4} speed={0.3} opacity={0.6} color="#ffd700" />
      <Sparkles count={Math.floor(200 * countScale)} scale={30} size={3} speed={0.2} opacity={0.4} color="#ffffcc" />

      {/* 中速闪烁的白色/银色星星 */}
      <Sparkles count={Math.floor(250 * countScale)} scale={20} size={2.5} speed={0.5} opacity={0.5} color="#ffffff" />
      <Sparkles count={Math.floor(150 * countScale)} scale={28} size={2} speed={0.4} opacity={0.3} color="#e6e6fa" />

      {/* 快速闪烁的彩色点缀 */}
      <Sparkles count={Math.floor(100 * countScale)} scale={18} size={2} speed={0.7} opacity={0.4} color="#ff6b6b" />
      <Sparkles count={Math.floor(100 * countScale)} scale={18} size={2} speed={0.65} opacity={0.4} color="#4ecdc4" />
